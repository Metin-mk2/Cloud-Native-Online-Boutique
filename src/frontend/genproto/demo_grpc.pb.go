...
// GiftWrappingServiceServer is the server API for GiftWrappingService service.
...
func (UnimplementedGiftWrappingServiceServer) GetGiftWrappingPrice(context.Context, *GiftWrappingPriceRequest) (*Money, error) {
	return nil, status.Error(codes.Unimplemented, "method GetGiftWrappingPrice not implemented")
}
...
func RegisterGiftWrappingServiceServer(s grpc.ServiceRegistrar, srv GiftWrappingServiceServer) {
	// If the following call panics, it indicates UnimplementedGiftWrappingServiceServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
